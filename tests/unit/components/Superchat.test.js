...
describe('Superchat Component', () => {
  ...
  it('should render superchat items correctly', () => {
    ...
    // 验证第一个超级弹幕显示正常
    expect(superchatItems[0].find('.superchat-nickname').text()).toBe(
      '测试用户'
    );
    expect(superchatItems[0].find('.superchat-message').text()).toBe(
      '这是一条超级弹幕'
    );
    expect(superchatItems[0].find('.superchat-fans').text()).toBe(
      '测试粉丝牌 Lv10'
    );
  ...
  it('should apply correct style based on price level', () => {
    ...
    // 验证粉丝牌和时间显示
    expect(highLevelItem.find('.superchat-fans').text()).toBe(
      '高级粉丝牌 Lv20'
    );
