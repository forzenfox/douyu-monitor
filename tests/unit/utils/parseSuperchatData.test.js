...
describe('Superchat Data Parsing', () => {
  ...
  it('should return correct header color by price', () => {
    ...
    // 测试价格5元（最低档）
    expect(getHeaderColor(5)).toBe(
      superchatOptions.find(opt => opt.minPrice === 0)?.bgColor?.header ||
        'rgb(21,101,192)'
    );

    // 测试价格10元（最低档）
    expect(getHeaderColor(10)).toBe(
      superchatOptions.find(opt => opt.minPrice === 0)?.bgColor?.header ||
        'rgb(21,101,192)'
    );

    // 测试价格30元（最低档）
    expect(getHeaderColor(30)).toBe(
      superchatOptions.find(opt => opt.minPrice === 0)?.bgColor?.header ||
        'rgb(21,101,192)'
    );

    // 测试价格50元（中档）
    expect(getHeaderColor(50)).toBe(
      superchatOptions.find(opt => opt.minPrice === 50)?.bgColor?.header ||
        'rgb(208,0,0)'
    );

    // 测试价格100元（中档）
    expect(getHeaderColor(100)).toBe(
      superchatOptions.find(opt => opt.minPrice === 50)?.bgColor?.header ||
        'rgb(208,0,0)'
    );

    // 测试价格500元（高档）
    expect(getHeaderColor(500)).toBe(
      superchatOptions.find(opt => opt.minPrice === 500)?.bgColor?.header ||
        'rgb(230,81,0)'
    );
  });
  ...
  it('should return correct body color by price', () => {
    ...
    // 测试价格5元（最低档）
    expect(getBodyColor(5)).toBe(
      superchatOptions.find(opt => opt.minPrice === 0)?.bgColor?.body ||
        'rgb(30,136,229)'
    );

    // 测试价格10元（最低档）
    expect(getBodyColor(10)).toBe(
      superchatOptions.find(opt => opt.minPrice === 0)?.bgColor?.body ||
        'rgb(30,136,229)'
    );

    // 测试价格30元（最低档）
    expect(getBodyColor(30)).toBe(
      superchatOptions.find(opt => opt.minPrice === 0)?.bgColor?.body ||
        'rgb(30,136,229)'
    );

    // 测试价格50元（中档）
    expect(getBodyColor(50)).toBe(
      superchatOptions.find(opt => opt.minPrice === 50)?.bgColor?.body ||
        'rgb(230,33,23)'
    );

    // 测试价格100元（中档）
    expect(getBodyColor(100)).toBe(
      superchatOptions.find(opt => opt.minPrice === 50)?.bgColor?.body ||
        'rgb(230,33,23)'
    );

    // 测试价格500元（高档）
    expect(getBodyColor(500)).toBe(
      superchatOptions.find(opt => opt.minPrice === 500)?.bgColor?.body ||
        'rgb(245,124,0)'
    );
  });
  ...
  it('should handle messages with missing fields', () => {
    ...
    const mockData =
      'vrid@=1234567890/chatmsg@=nn@A=测试用户@Stxt@A=测试内容@Scprice@=1000@Snow@=1705678901234@Stype@=comm_chatmsg@Srid@=123456@Sgbtemp@=2@Suid@=12345@ScrealPrice@=1000@Scet@=60@Sdanmucr@=1@S/type@=comm_chatmsg/rid@=123456/cprice@=1000/now@=1705678901234/gbtemp@=2/uid@=12345/crealPrice@=1000/cet@=60/danmucr@=1/';
