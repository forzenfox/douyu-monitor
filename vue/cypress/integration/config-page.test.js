...
describe('Config Page E2E Tests', () => {
  ...
  it('should update danmaku config settings', () => {
    ...
    // 验证设置已更新
    cy.get('.danmaku-size .van-slider__button').should('have.attr', 'style', /left: 30%/)
  })
  ...
  it('should update superchat config settings', () => {
    ...
    // 验证设置已更新
    cy.get('.superchat-keyword .van-field__control').should('have.value', '新关键词')
  })
  ...
  it('should save config and persist on reload', () => {
    ...
    // 验证配置已保存
    cy.get('.superchat-keyword .van-field__control').should('have.value', '测试关键词')
  })
