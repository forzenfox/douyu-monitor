<template>
  ...
  >
    ...
    <div
      ...
      :class="`item__fans ${!!data.diamond && showDiamond ? 'is-diamonds' : ''} FansMedal fansLevel-${data.fansLv}`"
    >
      <span class="FansMedal-name">{{ data.fansName }}</span>
      <img
        v-if="!!data.diamond && showDiamond"
    ...
    <span
      :style="showColor ? `color:${danmakuColor[data.color]};` : ''"
      class="item__txt"
      >{{ data.txt }}</span
    >
  </div>
