<template>
  <div
    ...
    :style="{
      backgroundColor: data.isExpired
        ? '#f0f0f0'
        : data.bgColor?.body || '#FF6B6B',
      '--superchat-header-color': data.isExpired
        ? 'transparent'
        : data.bgColor?.header || data.bgColor?.body || '#FF6B6B',
    }"
  >
    <!-- 前景色进度条 -->
    <div
      class="superchat-progress"
      :style="{
    ...
    <div class="superchat-content">
      ...
      >
        <!-- 粉丝牌 -->
        <span
          v-if="options?.superchat?.show?.includes('fans') && data.fansName"
          class="superchat-fans"
          :class="`fansLevel-${data.fansLv}`"
        >
          {{ data.fansName }}
        ...
        <!-- 贵族 -->
        <span
          v-if="options?.superchat?.show?.includes('noble') && data.noble"
          class="superchat-noble"
        >
          贵族
        </span>
        <!-- 房管 -->
        <span
          v-if="
            options?.superchat?.show?.includes('roomAdmin') && data.roomAdmin
          "
          class="superchat-roomAdmin"
        >
          房管
        </span>
        <!-- 钻粉 -->
        <span
          v-if="options?.superchat?.show?.includes('diamond') && data.diamond"
          class="superchat-diamond"
        >
          钻粉
        </span>
        <span
          class="superchat-nickname"
          :style="{
            color: data.isExpired ? '#999999' : data.nicknameColor || '#FFFFFF',
          }"
          >{{ data.nn || '匿名用户' }}</span
        >
        <span
          class="superchat-price"
          :style="{
            backgroundColor: data.isExpired
              ? '#CCCCCC'
              : data.bgColor?.header || data.bgColor?.body || '#FF6B6B',
        ...
        <!-- 时间 -->
        <span
          v-if="options?.superchat?.show?.includes('time')"
          class="superchat-time"
        >
