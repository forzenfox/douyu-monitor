<template>
  <div ref="domCommandDanmaku" class="command-danmaku">
    <div class="command-danmaku-list">
      ...
      ></Default>
      <div v-if="displayList.length === 0" class="empty-tip">暂无指令弹幕</div>
    </div>
    <div
      v-show="isLock"
      class="gobottom"
      @click.stop="
        goToScrollBottom(
          domCommandDanmaku.value?.querySelector('.command-danmaku-list')
        )
      "
    >
      回到底部
...
let { flexStyle, orderStyle, justifyContentStyle, textAlignStyle } =
  useFlexStyle(props, 'commandDanmaku');
...
// 监听弹幕列表变化，自动滚动到底部
watch(
  () => props.commandDanmakuList.length,
  (newLength, oldLength) => {
...
onUpdated(() => {
  const listDom = domCommandDanmaku.value?.querySelector(
