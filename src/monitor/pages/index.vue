<template>
  <div class="monitor" @click.prevent="onClickMonitor" ref="domMonitor">
    ...
    >
      ...
      <span class="status-text">{{
        isConnected ? '已连接' : `连接中...(${reconnectCount}次)`
      }}</span>
    </div>
    ...
    ></Gift>
    <Superchat
      v-if="options.switch.includes('superchat')"
      :maxOrder="maxOrder"
      :options="options"
    ...
    ></Danmaku>
    <CommandDanmaku
      v-if="options.switch.includes('commandDanmaku')"
      :maxOrder="maxOrder"
      :options="options"
      :commandDanmakuList="commandDanmakuList"
    ></CommandDanmaku>
  </div>
  <Popup
    class="popup"
  ...
  >
    <div class="popup-top">
      <div class="github">
        <a href="https://github.com/forzenfox/douyu-monitor" target="_blank"
          ><svg
            t="1636974784707"
            class="icon"
      ...
      <div v-if="options.isSaveData" @click="onClickSaveData">
        ...
        >
          <path
            d="M941.248 352L672 82.752A64 64 0 0 0 626.752 64H128a64 64 0 0 0-64 64v768a64 64 0 0 0 64 64h768a64 64 0 0 0 64-64V397.248A64 64 0 0 0 941.248 352zM256 128h48v160H256V128z m112 0H512v160h-144V128zM256 896v-192h512v192H256z m640 0h-64v-224a32 32 0 0 0-32-32H224a32 32 0 0 0-32 32v224H128V128h64v192a32 32 0 0 0 32 32h320a32 32 0 0 0 32-32V128h50.752L896 397.248V896z"
            p-id="3821"
            fill="#8a8a8a"
          ></path>
        </svg>
    ...
    <Tabs v-model:active="activeTab">
      ...
      <Tab title="弹幕">
        <Field label="占比">
          <template #input>
            <Slider
              v-model="options.size.danmaku"
              :disabled="maxOrder === options.order.danmaku"
            />
          </template>
        </Field>
        <Field label="显示">
        ...
        <Field
          ...
          label="屏蔽关键词"
          placeholder="空格隔开 例如:弹幕1 弹幕2"
        ></Field>
        <Field
          v-model="options.danmaku.ban.nicknames"
          label="屏蔽昵称"
          placeholder="模糊匹配 空格隔开 例如:昵称1 昵称2"
        ></Field>
        <Field label="过滤重复">
          <template #input>
      ...
      <Tab title="礼物">
        <Field label="占比">
          <template #input>
            <Slider
              v-model="options.size.gift"
              :disabled="maxOrder === options.order.gift"
            />
          </template>
        </Field>
        <Field
        ...
        ></Field>
        <Field
          v-model="options.gift.ban.fansLevel"
          label="粉丝牌等级≥"
          type="number"
          placeholder="屏蔽粉丝牌等级"
        ></Field>
      ...
      <Tab title="进场">
        ...
        <Field
          v-model="options.enter.keywords"
          label="关键昵称"
          placeholder="空格隔开 例如:昵称1 昵称2"
        ></Field>
        <Field
          v-model="options.enter.ban.level"
          label="屏蔽等级≤"
          type="digit"
          placeholder="请输入屏蔽的等级"
        ></Field>
      ...
      <Tab title="超级弹幕">
        <Field label="占比">
          <template #input>
            <Slider
              v-model="options.size.superchat"
              :disabled="maxOrder === options.order.superchat"
            />
          </template>
        </Field>
        <Field
          v-model="options.superchat.keyword"
          label="关键词"
          placeholder="请输入超级弹幕关键词"
        ></Field>
        <Field label="显示">
          <template #input>
            <CheckboxGroup
              v-model="options.superchat.show"
              direction="horizontal"
            >
              <Checkbox name="fans" shape="square">粉丝牌</Checkbox>
              <Checkbox name="noble" shape="square">贵族</Checkbox>
...
let activeTab = ref(0);
let { directionStyle, fontSizeStyle, avatarImgSizeStyle } =
...
onMounted(async () => {
  ...
  if (options.value.commandDanmaku) {
    ...
    if (options.value.commandDanmaku.keywords) {
      options.value.commandDanmaku.keywords =
        options.value.commandDanmaku.keywords.map(keyword => {
          // 如果颜色未定义或为黑色，使用默认颜色
...
function onClickShare() {
  ...
  Dialog.confirm({
    title: '复制分享链接',
    message:
      '链接保存了当前设置，可粘贴至斗鱼直播伴侣浏览器源中，使设置与网页一致',
  })
    .then(async () => {
      try {
